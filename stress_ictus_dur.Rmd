...
ggplot(shape_df,aes(x=shape,y=duration, color = ictus)) +
geom_violin() + stat_ydensity() + theme_minimal() +
...
```{r}
ggplot(shape_df,aes(x=shape,y=duration, color = stressed)) +
geom_violin() + stat_ydensity() + theme_minimal() +
labs( xlab = "syllable shape" ,ylab = "vowel duration (ms)") 
...
```{r}
linnullIntmod <- lmer(duration ~ (1|song) + (1|word) + (1|performer), data = strictus_data, REML = FALSE)
