...
%However, if one considers the findings along with an understanding of rhythmic entrainment, this conclusion is erroneous. 
...
%It would be difficult indeed for a singer to develop a novel phonology and subsequent motor control to sing in their own language: while it is certainly possible, as professional singers are able to learn to sing in languages they do not speak. Crucially, however, they have to spend time developing the new grammar, with both time and effort expended to reach the target.
%%

%Instead, the linguistic rhythmic system needs to entrain to a musical metrical pattern. 
...
%My aim here is to test this hypothesis by extending the findings of these earlier studies by looking at the segmental level. 
...
%
%The aim of this paper is to analyze the acoustic-phonetic realizations of metrics in Estonian lyrical folksongs known as {\it regilaul} (\(/re.ki.laul/\)). 
...
%Beginning in the early nineties and extending into the millenium, a collaboration between two native speakers of Estonian laid the groundwork for this study. 
Musicologist Jaan Ross and Phonologist Ilse Lehiste 

%found common ground investigating the acoustic correlates of regilaul: Ross had publications on one song \cite{ross1989, ross1992}, but the inclusion of a linguist in the study of lyrical realization proved crucial. 
...
%
%To extend the findings of this body of work, the present study examines temporal correlates of syllable weight and prominence at the segmental level: namely, the syllable nucleus. Because the vowel is the most sonorant part of a syllable, it is the most acoustically and perceptually salient portion of the syllable. Measuring the vowel duration will also offer indirect information about the rhyme as a whole: the presence of codas and complex codas should have an effect on the vowel duration in isochronous syllable-note sequences. In the cases where quantity is distinguished by coda consonant length, the syllable nucleus (measured by vowel duration) will be necessarily shorter to accommodate the geminate or complex coda. In cases where quantity is indicated by the length of the vowel, the opposite should be true. 
...
%\begin{itemize}
%\item long syllable-notes (on the beat)
%\begin{enumerate}
%	\item stressed syllables (longer)
%	\item unstressed syllables (shorter)
%\end{enumerate}
%\item short syllable-notes (off the beat)
%\begin{enumerate}
%	\item stressed syllables (longer)
%	\item unstressed syllables (shorter)
%\end{enumerate}
...
%	 
%
%	\ex \(HA\): duration contrasts for word-level stress will be evident at the segmental level after taking into account a syllables beat position in the song. 
%	\ex \(HA_{\emptyset}\): on or off-beat position of the song is best predictor for vowel duration of syllables in both stressed and unstressed syllables. 
%
%	\end{prop}
